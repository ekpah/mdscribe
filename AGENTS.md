...
- Prompt harnesses live under `apps/app/orpc/scribe/prompts/core/` and `apps/app/orpc/scribe/prompts/families/` as plain string fragments. Keep assembly simple: ordered string concatenation with blank lines, while registry/config lookup stays in `apps/app/orpc/scribe/prompts/`.
- For prompt families that use XML-structured instructions, keep one file per XML tag and compose them in the family entry file (like `families/narrative/discharge.ts`) so each prompt stays easy to scan and edit.
- Store tag fragments in a prompt-specific subfolder and use unprefixed tag filenames (e.g. `.../icu-transfer/system-role.ts`, `.../icu-transfer/quality-control.ts`), not flattened prefixed files like `icu-transfer-system-role.ts`.
