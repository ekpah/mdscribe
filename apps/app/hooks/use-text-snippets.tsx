...
export const useTextSnippets = () => {
	...
	// Find snippet by key
	const findSnippet = useCallback((key: string) => snippets.find((s) => s.key === key), [snippets]);
	...
	const expandSnippet = useCallback(
		(textarea: HTMLTextAreaElement | HTMLInputElement) => {
			...
			const key =
				lastSpaceIndex === -1 ? textBeforeCursor : textBeforeCursor.slice(lastSpaceIndex + 1);
			...
			const newText =
				textBeforeCursor.slice(0, lastSpaceIndex === -1 ? 0 : lastSpaceIndex + 1) +
				snippet.snippet +
				textAfterCursor;
