...
- Documents persist `DocumentTemplate.fieldDefinitions` and `pdfBytes`; derive Markdoc only via `buildParsedMarkdocFromFieldDefinitions`. Never persist document `parsedMarkdoc`/raw Markdoc.
