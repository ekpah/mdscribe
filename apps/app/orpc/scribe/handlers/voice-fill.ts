...
const normalizeVoiceFillObject = (object: unknown): z.infer<typeof voiceFillSchema> => {
	...
	if (object && typeof object === "object" && !("fieldValues" in object)) {
		...
		const allStrings = Object.values(record).every((value) => typeof value === "string");
		if (allStrings) {
...
const deriveFieldsFromTags = (inputTags: InputTagType[]): VoiceFillFieldDefinition[] => {
	...
	const pushField = (field: VoiceFillFieldDefinition) => {
		if (!field.label || seen.has(field.label)) {
			return;
		}
		fields.push(field);
...
const normalizeInputFields = (inputFields: InputField[] | undefined): VoiceFillFieldDefinition[] =>
