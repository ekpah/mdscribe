...
describe("Scribe usage logging", () => {
	...
	test("detects BYOK usage from the credential source", () => {
		expect(isByokUsageMetadata({ credentialSource: "user_byok" })).toBe(true);
		expect(isByokUsageMetadata({ credentialSource: "operator" })).toBe(false);
	});
});
