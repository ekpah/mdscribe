...
test.describe("BYOK settings", () => {
	test("authenticated users can open the write-only KI access dashboard", async ({
		page,
	}) => {
		await page.goto("/sign-in");
		const signInCard = page.getByTestId("sign-in-card");
		await signInCard
			.getByLabel("E-Mail oder Benutzername")
			.fill("test@test.com");
		...
		await expect(
			page.getByRole("heading", { name: "KI-Zugang" }),
		).toBeVisible();
		await expect(
			page.getByText(
		...
		await expect(
			page.getByRole("link", { exact: true, name: "KI-Zugang" }),
		).toBeVisible();
	});
});
