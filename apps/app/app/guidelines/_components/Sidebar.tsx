...
import { BookOpen, Minus, Plus, Search, StarIcon } from 'lucide-react';
...
const generateSegments = ({ guidelines }: { guidelines: Guideline[] }) => {
  const segments = guidelines.reduce<SidebarSegment[]>((acc, current) => {
    const category = current.category;
    const guideline = current.title;
    ...
    if (existingCategory) {
      ...
      acc.push({
        category,
        documents: [{ title: guideline, url: route, favouritesCount }],
      });
    }
...
export default function AppSidebar({
  ...
  guidelines: string;
  favouriteGuidelines: string;
  authoredGuidelines: string;
  isLoggedIn: boolean;
}) {
  const {
    state,
  ...
  const showCreateGuidelineButton = false;
  ...
  const menuSegments = JSON.parse(
    (() => {
      if (activeCollection === 'favourites') {
        return favouriteGuidelines;
      }
      if (activeCollection === 'authored') {
        return authoredGuidelines;
      }
      return guidelines;
    })()
  );
  ...
  const filteredLinks = fuse
    ...
    .sort(
      (a, b) =>
        (b as Guideline).favouritesCount - (a as Guideline).favouritesCount
    );

  // generate ordered segments to be visualized in the sidebar
  const orderedSegments = generateSegments({
    guidelines: searchTerm ? filteredLinks : menuSegments,
  });
