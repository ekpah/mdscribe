...
import { NavigationSidebar } from "@/app/_components/sidebar/navigation-sidebar";
import type {
...
const FavouriteCollectionIcon = (props: LucideProps) => <Bookmark {...props} strokeWidth={1.5} />;

const AuthoredCollectionIcon = (props: LucideProps) => <Pencil {...props} strokeWidth={1.5} />;
...
const generateSegments = ({ templates }: { templates: Template[] }) => {
	...
	for (const current of templates) {
		...
		const existingCategory = segments.find((segment) => segment.category === category);
		if (existingCategory) {
...
export default function AppSidebar({
	...
	const [activeCollection, setActiveCollection] = useQueryState("activeCollection", {
		defaultValue: isLoggedIn ? "favourites" : "visible",
	});

	const queryClient = useQueryClient();
	const initialTemplates = useMemo(() => JSON.parse(templates) as Template[], [templates]);
	...
	const customCollectionsData = customCollectionsQueryData ?? initialCustomCollections;
	...
	const collections = isLoggedIn ? [...userCollections, ...customCollectionEntries] : [];
	...
	}, [collectionForm.description, collectionForm.name, createCollectionMutation]);
	...
	const handleCollectionNameChange = useCallback((event: React.ChangeEvent<HTMLInputElement>) => {
	...
	const filteredLinks = fuse
		...
		.map((res) => res.item)
		.toSorted((a, b) => (b as Template).favouritesCount - (a as Template).favouritesCount);
							...
							<SidebarGroupContent className="relative">
								...
								>
									...
									<DialogContent>
										...
										<DialogFooter>
											<Button onClick={handleCloseCreateCollection} type="button" variant="outline">
												Abbrechen
											</Button>
											<Button
												...
												type="button"
											>
												{createCollectionMutation.isPending ? "Speichern..." : "Erstellen"}
											</Button>
										</DialogFooter>
									</DialogContent>
								</Dialog>
