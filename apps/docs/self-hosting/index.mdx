...
Das Docker-Image startet nur die App und fuehrt keine Migrationen beim Container-Start aus.

Wenn Sie MDScribe per Dockerfile ueber Coolify deployen, setzen Sie dafuer einen `Post-deployment`-Hook:
...
| `MAIL_SMTP_URL` | erforderlich | SMTP-Verbindung für Authentifizierungs- und System-E-Mails |
| `MAIL_FROM_ADDRESS` | erforderlich | Absenderadresse für alle E-Mails |
| `MAIL_FROM_NAME` | erforderlich | Anzeigename des Absenders |
| `MAIL_BROADCAST_SMTP_URL` | optional | Separater SMTP-Provider für Marketing-Broadcasts |
...
MDScribe verwendet Standard-SMTP und kann dadurch einen externen Provider oder
einen Relay in der eigenen Infrastruktur ohne Codeänderung anbinden.
...
`MAIL_SMTP_URL`. Zugangsdaten müssen URL-kodiert sein; die Zertifikatsprüfung
bleibt in Produktion aktiviert.
...
| MDScribe -> konfigurierter SMTP-Relay | Versand transaktionaler E-Mails | Anbieterwahl/Policy, Mail-Inhalte |
