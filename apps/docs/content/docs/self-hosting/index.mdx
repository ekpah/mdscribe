...
Vor dem ersten Produktivstart und bei jedem Schema-Change sollten die Drizzle-Migrationen ausgeführt werden:
...
## 8. Docker-Deployments und Migrationen

Wenn Sie MDScribe per Dockerfile deployen, fuehrt das Produktions-Image beim Container-Start automatisch die vorhandenen Drizzle-Migrationen aus und startet danach erst die App.

Manuelle Ausfuehrung gegen die produktive Datenbank:

```bash
bun run migrate
```

Direkt innerhalb eines laufenden Containers:

```bash
bun run --cwd packages/database migrate
```

Der automatische Schritt laesst sich fuer einen einzelnen Run mit `RUN_DB_MIGRATIONS_ON_START=false` deaktivieren.

## 9. Telemetrie- und Compliance-Hinweise
